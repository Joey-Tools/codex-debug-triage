...
- After an authoritative ZIP is local, use `scripts/archive_triage.py` with `references/local-artifact-recipes.md` for bounded member listing, selection, and text filtering. That helper performs no network or authentication work and does not replace the retained Jenkins acquisition entrypoint.
...
- Use `references/local-artifact-recipes.md` for the hard limits and safe command shapes of local ZIP inspection after acquisition.
