...
- Regex-worker teardown masks the repeating command-level `SIGALRM` across TERM/KILL/reap and pipe closure, restores the original deadline classification only after confirmed reap, and retains the authoritative process handle plus PID/PGID recovery evidence when reap cannot be proven.
...
- Review-fix regression selection: cleanup mode proof, workflow input identity, root DAC replacement, external regex-worker group termination, command deadline during regex teardown, and unreaped-worker handle retention.
- Python 3.13.0 full suite: 221 tests passed.
- System Python 3.9.6 full suite: 221 tests passed.
