...
- Hosted Linux fixed-curl integration now preserves the production fail-closed policy: a runner whose `/usr` trust component is owned by the runner account is accepted only as an exact `collector-unavailable` classification with zero transport launches, while a root-owned non-writable chain still exercises the successful request path.
