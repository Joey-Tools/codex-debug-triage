...
- The GitHub collector defers ordinary catchable termination signals across `Popen`, managed-handle construction, and registry publication; a deferred or post-publication signal reaps the child, removes the private credential snapshot, restores the caller signal state, and then forwards the original signal.
